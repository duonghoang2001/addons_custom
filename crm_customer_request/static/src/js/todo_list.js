/** @odoo-module */

import { Todo } from "./todo";
import { registry } from "@web/core/registry";
import { useAutofocus } from "./utils";
import { Component, useState } from "@odoo/owl";

export class TodoList extends Component {
    setup() {
        this.nextId = 0;
        this.todoList = useState([]);
        useAutofocus("todoListInput");
    }

    addTodo(ev) {
        if (ev.keyCode === 13 && ev.target.value != "") { // user pressed 'Enter' and todo is not empty
            this.todoList.push({ id: this.nextId++, description: ev.target.value, done: false });
            ev.target.value = "";
        }
    }

    toggleTodo(todoId) {
        const todo = this.todoList.find((todo) => todo.id === todoId);
        if (todo) {
            todo.done = !todo.done;
        }
    }

    removeTodo(todoId) {
        const todoIndex = this.todoList.findIndex((todo) => todo.id === todoId);
        if (todoIndex >= 0) {
            this.todoList.splice(todoIndex, 1);
        }
    }
}

TodoList.components = { Todo };
TodoList.template = "crm_customer_request.TodoList";

registry.category("fields").add("todo_list", TodoList);